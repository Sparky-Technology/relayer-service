...
func InstantSwap(c *gin.Context) {
	// 处理请求参数
	var bo *bo.InstantSwapBO
	if err := c.ShouldBindJSON(&bo); err != nil {
		// 请求参数有误，直接返回响应
		zap.L().Error("[InstantSwap] Invalid param!", zap.Error(err))
	...
	zap.L().Info("[InstantSwap] BO:", zap.String("bo", bo.ToString()))
	// 业务处理
	txHash, err := service.HandleInstantSwap(bo)
	if err != nil {
		zap.L().Error("[InstantSwap] Execution reverted!", zap.Error(err))
